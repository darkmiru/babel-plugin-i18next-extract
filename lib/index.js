...
function parseI18NextOptionsFromCommentHints(path, commentHints) {
    ...
    const userkeyCommentHint = getCommentHintForPath(path, 'USERKEY', commentHints);
    const res = {};
...
function referencesChildIdentifier(nodePath, parentNames, childName) {
    ...
    if (!obj.isIdentifier())
        return false;
    const prop = nodePath.get('property');
    if (Array.isArray(prop) || !prop.isIdentifier())
        return false;
...
function findJSXAttributeByName(path, name) {
    ...
    for (const attribute of attributes) {
        ...
        if (!attributeName.isJSXIdentifier())
            continue;
        if (name === attributeName.node.name)
            return attribute;
    }
    return null;
}
...
function resolveIdentifier(nodePath) {
    ...
    if (declarationExpressions.length === 0)
        return null;
    const latestDeclarator = declarationExpressions[declarationExpressions.length - 1];
    if (Array.isArray(latestDeclarator))
        return null;
    if (!latestDeclarator.isExpression())
        return null;
...
function parseTransComponentKeyFromAttributes(path) {
    const error = new ExtractionError(`Couldn't evaluate i18next key in Trans component. You should either ` +
        `make the i18nKey attribute evaluable or skip the line using a skip ` +
        `comment (/* ${COMMENT_HINTS_KEYWORDS.DISABLE.LINE} */ or /* ` +
        `${COMMENT_HINTS_KEYWORDS.DISABLE.NEXT_LINE} */).`, path);
    const keyAttribute = findJSXAttributeByName(path, 'i18nKey');
    if (!keyAttribute)
        return null;
    const keyAttributeValue = keyAttribute.get('value');
    const keyEvaluation = evaluateIfConfident(keyAttributeValue.isJSXExpressionContainer()
...
function formatJSXElementKey(path, index, config) {
    ...
    if (openingElement.get('attributes').length === 0 &&
...
function parseTransComponentKeyFromChildren(path, config) {
    const transComponentExtractionError = new ExtractionError(`Couldn't evaluate i18next key in Trans component. You should either ` +
    ...
    for (let [i, child] of children.entries()) {
        if (child.isJSXExpressionContainer()) {
            ...
            if (expression.isIdentifier()) {
                ...
                else if (declarationExpression !== null &&
...
function extractTransComponent(path, config, commentHints = [], skipCheck = false) {
    if (getCommentHintForPath(path, 'DISABLE', commentHints))
        return [];
    if (!skipCheck && !isTransComponent(path))
        return [];
 ...
 */
function isCustomTransComponent(absoluteCustomTransComponents, path$1) {
    const openingElementName = path$1.get('openingElement').get('name');
    return absoluteCustomTransComponents.some(([sourceModule, importName]) => {
        if (path.isAbsolute(sourceModule)) {
            ...
            if (referencesImport(openingElementName, relativeSourceModulePath, importName)) {
                return true;
            }
        }
...
function extractCustomTransComponent(path, config, commentHints = []) {
    if (getCommentHintForPath(path, 'DISABLE', commentHints))
        return [];
    if (!isCustomTransComponent(config.cache.absoluteCustomTransComponents, path))
        return [];
...
function extractTCall(path, commentHints) {
    ...
    if (typeof keyEvaluation !== 'string') {
        throw new ExtractionError(`Couldn't evaluate i18next key. You should either make the key ` +
...
function findWithTranslationHOCCallExpressionInParents(path) {
    ...
    if (Array.isArray(callee) || !callee.isCallExpression())
        return null;
    return callee;
}
/**
...
function findWithTranslationHOCCallExpression(path) {
    ...
    for (const refPath of bindings.referencePaths) {
        const callee = findWithTranslationHOCCallExpressionInParents(refPath) ||
...
function extractGetFixedTFunction(path, config, commentHints = []) {
    if (!isGetFixedTFunction(path, config))
        return [];
    let ns;
    const nsCommentHint = getCommentHintForPath(path, 'NAMESPACE', commentHints);
    if (nsCommentHint) {
 ...
 *   already is a `t` function.
 */
function extractUserKey(path, config, commentHints = []) {
    if (!path.node.loc)
        return [];
    if (getCommentHintForPath(path, 'DISABLE', commentHints))
        return [];
    ...
    if (parsedOptions.hasCount) {
        ...
        if (config.enableExperimentalIcu) {
            ...
            const icuPlurals = pluralNumbersAsText
                .map((numAsText) => `${numAsText} {${icuPluralValue(extractedKey.parsedOptions.defaultValue)}}`)
                .join(' ');
            ...
            if (numberOfPlurals === 1) {
                ...
                keys = keys.reduce((accumulator, k) => [
                    ...accumulator,
                    k,
                    {
                        ...k,
...
function parseConfig(opts) {
    ...
    return {
        ...
        cache: {
            absoluteCustomTransComponents: customTransComponents.map(([sourceModule, importName]) => [
...
const Visitor = {
    ...
    ObjectProperty(path, state) {
        const extractState = this.I18NextExtract;
        handleExtraction(path, state, collect => {
            collect(Extractors.extractUserKey(path, extractState.config, extractState.commentHints));
        });
    },
